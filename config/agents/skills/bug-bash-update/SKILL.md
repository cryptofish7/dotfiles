...
1. **Test in the browser.** Verification happens via Codex in Chrome MCP tools (navigate, click, screenshot, read page). `forge test`, `cast`, `pnpm test`, and code review do NOT count. If the extension isn't connected, stop and tell the user.
