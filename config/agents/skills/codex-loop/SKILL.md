...
Run Codex as a lead-architect reviewer in a convergence loop: Codex flags blockers, Codex fixes them, repeat until the target is approved or the loop stops.
...
2. **Run Codex non-interactively**:
   ```bash
   cat <<'PROMPT' | codex exec -m gpt-5.6-sol -c model_reasoning_effort="high" -
   <prompt body>
   PROMPT
   ```
...
2. Scan the plan for file paths, function names, module references. Read 5–10 directly-referenced files. Also read `AGENTS.md` if present.
...
<INSERT RELEVANT FILE CONTENTS AND AGENTS.md CONVENTIONS>
...
<INSERT AGENTS.md CONTENTS IF PRESENT>
