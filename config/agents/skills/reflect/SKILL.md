...
|---|---|
| Mistakes to avoid | `AGENTS.md` "Mistakes to Avoid" section (append) |
| Skill opportunities | `~/.Codex/skills/<name>/SKILL.md` (create stub) |
| Skill improvements | Edit existing skill's `SKILL.md` |
| Memory candidates | `~/.Codex/projects/<path>/AGENTS.md` (append) |
| Process improvements | Edit `AGENTS.md` workflow sections |
...
- **Mistakes to avoid:** Append to the "Mistakes to Avoid" section of the project's `AGENTS.md`.
...
- **Memory candidates:** Append to `~/.Codex/projects/<path>/AGENTS.md`. Create the file if it doesn't exist.
- **Process improvements:** Show the before/after for the affected AGENTS.md section. Edit in place.
...
- **Memory candidates must be non-obvious.** Don't propose things already captured in project docs, config files, or AGENTS.md.
- **Don't self-reference.** This skill should never propose adding itself as a finding.
- **Respect scope.** Project-specific knowledge goes to project memory. Universal lessons go to AGENTS.md. Don't mix them.
