---
name: fable
description: Review a plan file or a PR/branch diff using a fresh Claude subagent pinned to the Fable model. Use when the user says "/fable" (with or without an argument) or asks to review something with Fable. Auto-detects plan-review vs PR-review mode from the argument and conversation context.
---

# Fable Review

Use a fresh Claude subagent pinned to the **Fable** model as a second set of eyes — either on an implementation plan (before work begins) or on a PR / branch diff (after work is done). Because the reviewer runs on a different model with clean context, it's an independent perspective, not an echo of the current session.
...
2. Scan it for file paths, function names, or module references. Note the key ones (up to 5–10) so you can point the subagent at them. Also note whether `CLAUDE.md` exists.
...
<INSERT LIST OF FILE PATHS THE PLAN REFERENCES; note if CLAUDE.md exists so the reviewer reads project conventions>
...
<INSERT LIST OF FILE PATHS; note if CLAUDE.md exists so the reviewer reads project conventions>
